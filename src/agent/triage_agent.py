...
: decision,
: {
: len(search_context['similar_tickets']),
: len(search_context['kb_articles']),
: search_context['customer_history']
            },
: esql_analysis,
: workflow_result,
: self._generate_response(ticket, search_context, decision),
: 0                                
        }
...
, 'down', 'not working', 'broken', 'cant', 'cannot',
, 'crashed', 'error', 'failing']
...
: [
            ...
            return [
                {
: hit["_source"]["ticket_id"],
: hit["_source"]["subject"],
: hit["_source"]["category"],
: hit["_source"]["priority"],
: hit["_source"].get("resolution_time_minutes"),
: hit["_score"]
                }
...
: satisfaction
    ...
    def _classify_by_keywords(self, ticket: Dict) -> str:
        ...
        category_keywords = {
: ['error', 'crash', 'bug', 'not working', 'broken', 'api', 'integration', 'performance', 'slow'],
: ['charge', 'payment', 'invoice', 'refund', 'subscription', 'billing', 'credit card'],
: ['login', 'password', 'access', 'account', 'email', 'locked', 'reset'],
: ['request', 'feature', 'suggestion', 'would like', 'can you add', 'need']
        }
...
: needs_human_review,
: {
: esql_analysis['factors'],
: len(context['similar_tickets']),
: len(context['kb_articles'])
            }
        }
...
: datetime.now().isoformat()
    ...
    def _generate_response(self, ticket: Dict, context: Dict, decision: Dict) -> str:
        ...
        if kb_articles:
            ...
            response = f"Thank you for contacting support. Based on your issue regarding '{subject}', "                      f"we've categorized this as a {decision['category']} issue with {decision['priority']} priority. "                      f"\n\nYou might find this helpful: {article['title']} (Article {article['article_id']})"                      f"\n\nOur {decision['assigned_team']} team will review your ticket shortly."
        else:
            response = f"Thank you for contacting support. We've received your ticket regarding '{subject}'. "                      f"This has been categorized as a {decision['category']} issue with {decision['priority']} priority. "                      f"Our {decision['assigned_team']} team will get back to you soon."
